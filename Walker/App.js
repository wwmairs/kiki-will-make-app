...
export class LoginScreen extends React.Component {
    ...
    render() {
        ...
        return ( <
            ...
            <
            Button onPress = {
                () => navigate("Signup", { name: this.state.email }) } style = { styles.buttonText } title = "SIGNUP"
            ...
            onPress = { this.onLoginPress.bind(this) } style = { styles.buttonText } title = "LOGIN"
            accessibilityLabel = "Click this shit to login" /
            >
            <
            /TouchableOpacity> <
...
export class SignupScreen extends React.Component {
    ...
    onSignUp() {
        ...
        const { email, password } = this.state;
        console.log(
            "im here in onSignUp, email is:" + email + "and password is: " + password
        );
        //Login was not successful, let"s create a new account
        firebase
            .auth()
    ...
    render() {
        ...
        return ( <
            ...
            /> <
            TextInput style = { styles.input } placeholder = { this.state.email } placeholderTextColor = "rgba(255,255,255,0.7)"
            onChangeText = { email => this.setState({ email }) } value = { this.state.email }
            /> <
            TextInput style = { styles.input } placeholder = "password"
            placeholderTextColor = "rgba(255,255,255,0.7)"
            secureTextEntry onChangeText = { password => this.setState({ password }) } value = { this.state.password }
            />
            ...
            onPress = { this.onSignUp.bind(this) } style = { styles.buttonText } title = "Sign me up baby!!"
            accessibilityLabel = "Click this shit to login" /
            >
            <
            /TouchableOpacity> <
...
export class DirectionsScreen extends React.Component {
    ...
    render() {
        ...
        return ( <
            ...
            TextInput style = { styles.input } placeholder = "Where are you headed?"
...
export class ContactsScreen extends React.Component {
    ...
    render() {
        ...
        return ( <
            ...
            <
            TextInput style = { styles.input } placeholder = { "Search for contacts to notify" } onChangeText = { name => this.updateContactsList(name) }
            /> <
            ListView style = { styles.contactList } dataSource = { this.state.dataSource } renderRow = {
                rowData => ( <
                    TouchableHighlight underlayColor = "#008b8b"
                    onPress = { this.addSelectedContact.bind(this, rowData) } >
                    <
            ...
            ListView style = { styles.contactList } dataSource = { this.state.selectedDataSource } renderRow = {
                rowData => ( <
                    TouchableHighlight underlayColor = "#008b8b"
                    onPress = { this.removeSelectedContact.bind(this, rowData) } >
                    <
                    Text > { rowData.givenName } < /Text> <
                    /TouchableHighlight>
                )
            ...
            /> <
            /View> <
            Button style = { styles.startButton } onPress = {
                () => {
                    this.itemsRef.push({
                        dest: this.state.end,
...
export class MapScreen extends React.Component {
    ...
    constructor(props) {
      ...
      "," +
      origin.longitude}&destination=${destination}&key=${APIKEY}&mode=${mode}`;
    ...
    updatePosition() {
        ...
        navigator.geolocation.getCurrentPosition(
            position => {
                ...
                const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${origin.latitude +
          "," +
          origin.longitude}&destination=${destination}&key=${APIKEY}&mode=${
          mode
        }`;
                fetch(url)
                    .then(response => response.json())
                    .then(responseJson => {
                        ...
                        if (
                            responseJson.routes[0].legs.length == 1 &&
    ...
    render() {
        ...
        return ( <
            ...
            MapView style = { styles.map } region = { this.state.region } >
            <
            MapView.Polyline coordinates = { this.state.coords } strokeWidth = { 4 } strokeColor = { "#42aaf4" }
            /> <
            MapView.Marker coordinate = {
                {
                    latitude: this.state.region.latitude,
